    ...
    size_t len)
{
    ...
    ic_card_on_rx_event_isr(device, (uint16_t)len);
    ic_card_process(device);
}
...
int main(void)
{
    ...
    assert(ic_card_read_block_key_a(&device, 0x20U, 0x01U, true) == IC_CARD_OK);
    ...
    success[21] = ic_card_checksum(success, 21U);
    fake_feed(&device, &fake, success, 7U);
    fake_feed(&device, &fake, &success[7], 15U);
    assert(ic_card_take_response(&device, &sequence, &response));
    assert(ic_card_extract_block_data(&response, 0x20U, block) == IC_CARD_OK);
    ...
    assert(ic_card_build_read_block_key_a_frame(
    ...
    assert(ic_card_build_query_frame(
    ...
    assert(ic_card_parse_response_frame(
