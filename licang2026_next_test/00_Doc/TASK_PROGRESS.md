...
## 0.8 2026-09-24 绕柱粗调与仓库点位初调

- 小圆盘绕柱使用起点`(1475,2595)`、半径`-318 mm`、速度`180 mm/s`和理论约`1.15`圈的定时绕行；已进行现场粗调，动作组、起点/圆心及末段位置仍待完善，不记作完整视觉抓取验收。
- 仓库D1~D4目标Y初调为`2208/2407/2608/2793 mm`。后续重点复测逐点横移、重复停点和回家路线；正式仓库数字映射、放球及倒垛尚未接入。
- 无线测试模式已允许机械臂缺席时执行纯路径，但抓球`ROUTE`仍要求动作组10完成。现场状态显示动作组10完成回报已到；测试任务空轮询误判CMSIS错误值的问题已修正，待切回测试模式重新烧录复测。当前开关为0，运行正式Mission。
- 正式模式下曾出现阶梯中层未走完即转入小圆盘，用户确认1号红外接触不良。该红外负责判断车身大部分已离开阶梯，不承担找线定位；当前代码以其高电平结束中层。连接修复后的整段回归仍待完成，不能把此次提前结束归因于视觉。

## 0.7 2026-09-23 阶梯现场调参完成

- 阶梯底盘最终现场参数为横移`-60 mm/s`、切层视觉就绪后稳定等待`500 ms`、低层到高层切换Y=`2750 mm`、高层到中层切换Y=`2400 mm`。
- Nano端低层与高层ROI已按现场抓取画面调整，低层形状门槛已适当放宽，低/高/中三层均使用单帧确认；对应视觉提交已经推送至视觉仓库分支。
- 用户确认当前STM32与Nano参数组合能够完成阶梯视觉停车和机械臂夹取。未单独记录红蓝两色、三层逐项和多轮重复次数，因此不把本次成功扩大为所有组合的可靠性验收。

## 0.6 2026-09-21 追线现场调参与主线发布停点

- 用户将AL_MID_YAW_OFS由-3.0f改为-1.56f，反馈回中效果改善；参数单独提交为cdf5239，不代表各路线、仓库停点或HOME_DIRECT已全部验证。
- 当前使用O3，无线测试开关为1；用户已反馈上板运行、动作组10执行和无线命令可用，未提供完整命令记录或O3性能对比。
- 用户授权上传GitHub main；推送因github.com:443连接失败而未确认成功。此前origin/licnag2026_E已推送至f570dae，main本地跟踪引用仍为8adb900，后续需联网核对。
- Keil仅将相关子分组合并为01_App、02_Service、03_Middleware、04_Bsp、lhy；111个文件条目、源码路径与编译选项保持不变。工程树整理和原下载器配置仍未提交，不得和RTE_Components.h等本地修改自动带入主线。
- 下一步逐条进行底盘和仓库点位调参，记录方向、终点、X/Y与航向偏差、速度、过冲、停车、危险区域和DONE/FAULT；之后进行Nano正式响应器与F7单目标视觉闭环。
...
- 现有底盘整段测试已改为独立USART1无线任务，正式`mission_task_entry()`未修改。纯路径模式按命令逐段走转盘、阶梯、小圆盘和仓库；单目标模式只在指定转盘、阶梯或小圆盘区域执行完整视觉抓取，目标完成后不走后续路线。
- 跳过区域结束等待2秒；仓库支持`D1~D4`逐点移动，`HOME`先回1号位再回家，`HOME_DIRECT`仅在仓库内从当前位置直接执行回家路线。无线端可查询状态和球档案，存球后自动打印记录。
- 加入`HOME_DIRECT`后的无线测试构建和此前临时恢复正式Mission的构建均为0错误、0警告，球档案Host测试通过；尚未验证USART1无线模块、D2~D4直接回家、底盘停点、机械臂动作、Nano/F7会话或整机流程。
...
- 下一阶段转为现场调参：先测纯底盘路径和仓库点位，再用Nano `--show-masks`配合单目标模式分别调整转盘、阶梯和小圆盘视觉。F756上板基础验证、O3和正式仓库业务继续后置。
